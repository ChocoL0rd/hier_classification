...
class TrainCLSTreeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idxs = self.tokenizer.rows2idxs(self.tokenizer.pad_complement(row))
        masked_idxs, target = self.tokenizer.random_mask_idxs(idxs)

        return img_path, img, torch.from_numpy(masked_idxs), target
